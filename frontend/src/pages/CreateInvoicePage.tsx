...
export const CreateInvoicePage: React.FC = () => {
  ...
  // Invoice details with shop margin and discount percentage
  const [invoiceDetails, setInvoiceDetails] = useState({
    due_date: '',
    tax_amount: 0,
    discount_percentage: 0, // Changed from discount_amount to discount_percentage
    ...
    try {
      ...
      if (userRole === 'owner' || userRole === 'developer') {
        ...
        stockData = availableProducts.map(product => ({
          ...
          batch_cost: product.batch_cost || product.base_price,
  ...
  const handleAddProductFromModal = () => {
    ...
    const newItem: InvoiceItem = {
      ...
      batch_id: productSelection.product.batch_id,
      batch_cost: productSelection.product.batch_cost || productSelection.product.product_base_price,
  ...
  const handleQuickAddProduct = (product: SalesmanStock) => {
    ...
    const newItem: InvoiceItem = {
      ...
      product,
      quantity: 1,
      unit_price: product.batch_cost || product.product_base_price || 0,
      batch_id: product.batch_id,
      batch_cost: product.batch_cost || product.product_base_price,
      total: 1 * (product.batch_cost || product.product_base_price || 0),
    };
  ...
  const discountAmount = subtotal * (invoiceDetails.discount_percentage / 100); // Calculate discount from percentage
  const discountedTotal = subtotal - shopMarginAmount;
  const finalTotal = discountedTotal + invoiceDetails.tax_amount - discountAmount;
    ...
    try {
      ...
      if (userRole === 'salesman') {
        ...
        const simplifiedInvoiceData: CreateSimplifiedInvoiceData = {
          ...
          discount_amount: discountAmount, // Convert percentage to amount for API
          ...
          items: invoiceItems.map(item => ({
            ...
            batch_cost: item.batch_cost || item.unit_price,
          })),
        };
        ...
        const invoiceData: CreateInvoiceData = {
          ...
          due_date: invoiceDetails.due_date || undefined,
          tax_amount: invoiceDetails.tax_amount,
          discount_percentage: invoiceDetails.discount_percentage, // Use discount percentage
          shop_margin: invoiceDetails.shop_margin,
          notes: invoiceDetails.notes,
          terms_conditions: invoiceDetails.terms_conditions,
          items: invoiceItems.map(item => ({
            ...
            effective_shop_margin: invoiceDetails.shop_margin + invoiceDetails.discount_percentage, // Combined margin
          })),
        };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-4">
          <div className={`flex items-center space-x-2 ${selectedShop ? 'text-green-600' : 'text-blue-600'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${selectedShop ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
              }`}>
              {selectedShop ? '✓' : '1'}
            </div>
          ...
          <div className={`flex items-center space-x-2 ${selectedShop && invoiceItems.length > 0 ? 'text-green-600' :
              selectedShop ? 'text-blue-600' : 'text-gray-400'
            }`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${selectedShop && invoiceItems.length > 0 ? 'bg-green-100 text-green-800' :
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`input ${userRole === 'salesman' && invoiceDetails.shop_margin > maxShopMargin
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="0"
                          max="100"
                          value={invoiceDetails.discount_percentage}
                          onChange={(e) => setInvoiceDetails(prev => ({ ...prev, discount_percentage: parseFloat(e.target.value) || 0 }))}
                          className="input"
                          placeholder="0.0"
                        />
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                          Additional discount percentage added to shop margin
                        </p>
                      </div>
                      <div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-600 dark:text-gray-400">Discount ({invoiceDetails.discount_percentage}%):</span>
